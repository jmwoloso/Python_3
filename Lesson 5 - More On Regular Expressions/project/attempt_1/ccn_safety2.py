...
def ccn_safety(text):
    """
    Obscures the first 12 numbers for one or more credit card numbers
    in a body of text
    """
    # thanks for the awesome named group concat trick Kirby!
    return re.subn(r"(\d{4}-){3}(?P<last_four>\d{4})", 
                   "XXXX-XXXX-XXXX-"+"\g<last_four>", 
                   text)
